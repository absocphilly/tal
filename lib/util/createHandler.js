module.exports = d => {
	...
	function createHandler(options, handler) {
		...
		if (!handler) throw new d.errors.SystemError('A "handler" function must be provided to createHandler options.');
		...
		function refreshOptions(options) {
			// Load
			handler     = options.handler    === undefined ? handler              : options.handler;
		...
		function rotateCollection() {
			...
			lastPromise = Promise.resolve(concurrent ? undefined : lastPromise)
				.then(async () => {
					try {
						await handler(oldRequests, options);
	...
	class HandlerRequestCollection {
		 ...
		 * Returns a promise that will resolve/reject once the handler has completed
		...
		/**
		 * Returns all unresolved element options in the collection.
		 */
		getElementOptions() {
			let optionsArray = [],
			...
			for (let i = 0; i < elements.length; i++) {
				...
				if (!element._isComplete) optionsArray.push(element.options);
			}
			return optionsArray;
		}
		 ...
		 * elements with the element. If defined, it will have the same signature
		...
		resolveAll(handler) {
			...
			return that.operation = that.operation.then(() => {
				...
				for (let i = 0; i < elements.length; i++) {
					...
					// Handle
					try {
						let result = !handler ? element : handler.call(context, element.options, element.resolve, element.reject, i);
						promises[i] = Promise.resolve(result).reflect();
					}
					catch (e) {
						promises[i] = Promise.reject(e).reflect();
